...
class BlurIdentification(object):
    ...
    def __call__(self, results):
        ...
        mh,mw = (h-8)%self.row, (w-8)%self.col
        hl,hr = mh//2,mh-mh//2
        wt,wb = mw//2,mw-mw//2
        crop = tensor[hl+4:h-hr-4,wt+4:w-wb-4]
        input = togray(crop)
        data = gridImg(input,self.row,self.col)
        gass_w = gaussion(3,5).cuda()
        weight = laplacian(0).cuda()
        out = torch.abs(conv2d(conv2d(data,gass_w),weight))
        var = torch.sort(out.reshape(out.size(0),-1).var(1))[0]
...
@PIPELINES.register_module()
class RemoveBlur(object):
    def __init__(self,blur_score=8):
    ...
    def __call__(self, results):
        ...
        img_1 = image[10:h_+10,xmin:xmax]
        img_2 = image[h-10-h_:h-10,xmin:xmax]
        img_3 = image[ymin:ymax,10:w_+10] 
        img_4 = image[h-10-h_:h-10,w-10-w_:w-10]         
        
        var = cv2.Laplacian(img, cv2.CV_32F).var()
        v1 = cv2.Laplacian(img_1, cv2.CV_32F).var()
        v2 = cv2.Laplacian(img_2, cv2.CV_32F).var()
        v3 = cv2.Laplacian(img_3, cv2.CV_32F).var()
        v4 = cv2.Laplacian(img_4, cv2.CV_32F).var()
        
        blur_min = min(var,v1,v2,v3,v4)
        blur_max = max(var,v1,v2,v3,v4)
        blur_diff = (blur_max-blur_min)

        results['blur'] = False
        results['blur_diff'] = blur_diff
        results['blur_value'] = blur_min
        
        if (blur_min<10 and blur_min>4 and blur_diff<3.5) or (blur_min<4 and blur_diff<1):
            results['blur'] = True 
        return results

    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += f'(blur_score={self.blur_score})'
        return repr_str
...
class Resize(object):
    ...
    def __init__(self,
        ...
        if img_scale is None:
            ...
            if isinstance(img_scale, list) and isinstance(img_scale[0], int):
                self.img_scale = [tuple(img_scale),]
            else:
                self.img_scale = [tuple(sc) for sc in img_scale]
...
@PIPELINES.register_module()
class BrightAndContrast(object):
    """Apply photometric distortion to image sequentially, every transformation
    is applied with a probability of 0.5. The position of random contrast is in
    second or second to last.
    ...
    def __call__(self, results):
        ...
        assert img.dtype == np.float32, \
            'Brightness needs the input image of dtype np.float32,'\
            ' please set "to_float32=True" in "LoadImageFromFile" pipeline'
        # random brightness
        if random.randint(2):
            delta = random.uniform(-self.brightness_delta,
...
class GammaTransform(object):
    """Apply gamma transformation to image, to augment the contrast of the image of different brightness.
    """

    def __init__(self,
                 gamma_ratio=0.5,min_p=80,max_p=220,mean_p=120):
    ...
    def __call__(self, results):
        ...
        if 'gamma' not in results:
            gm = True if np.random.rand() < self.gamma_ratio else False
            if self.gamma_ratio == 0.0: 
                gm = False
            results['gamma'] = gm
        if results['gamma']:
            p1 = self.mean_p-(self.mean_p-self.min_p)/2
            p2 = self.mean_p+(self.max_p-self.mean_p)/2
            mm = img.mean()
            if mm<p1:
                gamma = random.choice([0.4,0.5,0.6,0.7,0.8,1.1,1.2,1.3])
            elif mm<p2:
                gamma = random.choice([0.6,0.7,0.8,0.9,1.2,1.4,1.6,1.8,2.0])
            else:
                gamma = random.choice([0.7,0.8,0.9,1.2,1.4,1.6,1.8,2.0,2.2,2.4,2.5])

            lookUpTable = (np.power((np.arange(256)+0.5)/256.0,gamma)*256-0.5).astype(np.uint8).reshape((1,-1))
    ...
    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += f'(\ncontrast_gamma={self.gamma})'
        return repr_str


@PIPELINES.register_module()
class ColorTransfer(object):
    """Apply color transfer from the current image to des image, to augment the color of the image.
    """

    def __init__(self,transfer_ratio=0.3):
    ...
    def __call__(self, results):
        ...
        if 'img_fields' in results:
            assert results['img_fields'] == ['img'], \
                'Only single img_fields is allowed'
        img = results['img']
        if 'color_transfer' not in results:
            transfer = True if np.random.rand() < self.transfer_ratio else False
            if self.transfer_ratio == 0.0: 
                transfer = False
            results['color_transfer'] = transfer
        if results['color_transfer']:
            indx = random.randint(len(results['img_infos']))
            des_path = results['img_prefix']+results['img_infos'][indx]['filename']
            des_img = cv2.imread(des_path)
            m_ = img[:,:,0].mean(),img[:,:,1].mean(),img[:,:,2].mean()
            s_ = img[:,:,0].std(),img[:,:,1].std(),img[:,:,2].std()

            dm_ = des_img[:,:,0].mean(),des_img[:,:,1].mean(),des_img[:,:,2].mean()
            ds_ = des_img[:,:,0].std(), des_img[:,:,1].std(), des_img[:,:,2].std()
            img = (img - m_)/s_*ds_+dm_
            img = np.clip(img,0,255).astype(np.uint8)
            results['color_transfer_img'] = des_path
        results['img'] = img
        return results

    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += f'(\ncolor_transfer_ratio={self.transfer_ratio})'
        return repr_str
